    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Hari Libur</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Isi formulir berikut untuk menambahkan hari libur baru.</p>
            </div>

            <!-- Form Card -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <form action="{{ route('admin.holidays.store') }}" method="POST" class="p-6 md:p-8 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Hari Raya Idul Fitri"
                            class="w-full rounded-xl border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                        @error('name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Tambahkan keterangan tambahan..."
                            class="w-full rounded-xl border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_national_holiday" id="is_national_holiday" value="1" {{ old('is_national_holiday', 1) ? 'checked' : '' }}
