        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                <form method="POST" action="{{ route('admin.roles.update', $role) }}">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2" for="system_role_id">
                            System Access Level
                        </label>
                        <select id="system_role_id" name="system_role_id"
                            ...
                            @foreach($systemRoles as $systemRole)
                                <option value="{{ $systemRole->id }}" {{ old('system_role_id', $role->system_role_id) == $systemRole->id ? 'selected' : '' }}>
                                    {{ $systemRole->name }} ({{ ucfirst($systemRole->slug) }} Access)
                                </option>
                            @endforeach
                        </select>
                        <p class="text-gray-600 text-xs italic mt-1">Determines if users with this role can access the admin dashboard.</p>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4 flex items-center">
                        <input id="is_active" name="is_active" type="checkbox" value="1" {{ $role->is_active ? 'checked' : '' }}
